...
decode(const std::string& nums, int pos, int end) {
    ...
    for(auto i = pz; i < end; i++) {
        ...
        FC_ASSERT(c != nullptr, "invalid character in vros-link");
        num *= 42;
        num += (c - ALPHABETS);
    }
...
fc::flat_map<uint8_t, vros_link::segment>
parse_segments(const bytes& b, uint16_t& header) {
    ...
    auto segs = fc::flat_map<uint8_t, vros_link::segment>();

    auto i  = 2u;
    auto pk = 0u;
    while(i < b.size()) {
        auto k = (uint8_t)b[i];
        vros_ASSERT(k > pk, vros_link_exception, "Segments are not ordered by keys");
        pk = k;

        if(k <= 20) {
            FC_ASSERT(b.size() > i + 1); // value is 1 byte
            auto v = (uint8_t)b[i + 1];
            segs.emplace(k, vros_link::segment(k, v));
            ...
            segs.emplace(k, vros_link::segment(k, boost::endian::big_to_native(v)));
            ...
            if(sz > 0) {
                ...
                segs.emplace(k, vros_link::segment(k, std::string(&b[i] + 1 + s, sz)));
            }
            else {
                segs.emplace(k, vros_link::segment(k, std::string()));
            }

            i += 1 + s + sz;
        }
        else {
            vros_THROW(vros_link_exception, "Invalid key type: ${k}", ("k",k));
        }
    }
    return segs;
}
...
vros_link
vros_link::parse_from_vrosli(const std::string& str) {
    using namespace __internal;

    vros_ASSERT(str.size() < 400, vros_link_exception, "Link is too long, max length allowed: 400");
    vros_ASSERT(str.size() > 20, vros_link_exception, "Link is too short");

    size_t start = 0;
    const char* uri_schema = "https://vros.li/";
...
const vros_link::segment&
vros_link::get_segment(uint8_t key) const {
    auto it = segments_.find(key);
    vros_ASSERT(it != segments_.end(), vros_link_no_key_exception, "Cannot find segment for key: ${k}", ("k",key));

    return it->second;
}

bool
vros_link::has_segment(uint8_t key) const {
    return segments_.find(key) != segments_.end();
}

fc::sha256
vros_link::digest() const {
...
void
vros_link::add_segment(const segment& seg) {
...
void
vros_link::add_signature(const signature_type& sig) {
    signatures_.emplace(sig);
}

void
vros_link::sign(const private_key_type& pkey) {
...
using vros::chain::contracts::vros_link;

void
from_variant(const fc::variant& v, vros_link& link) {
    link = vros_link::parse_from_vrosli(v.get_string());
}

void
to_variant(const vros::chain::contracts::vros_link& link, fc::variant& v) {
