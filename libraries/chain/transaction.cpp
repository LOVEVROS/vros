...
transaction::get_signature_keys(const vector<signature_type>& signatures, const chain_id_type& chain_id,
    ...
    try {
        ...
        for(const signature_type& sig : signatures) {
            ...
            vros_ASSERT(allow_duplicate_keys || successful_insertion, tx_duplicate_sig,
...
packed_transaction::get_unprunable_size() const {
    ...
    vros_ASSERT(size <= std::numeric_limits<uint32_t>::max(), tx_too_big, "packed_transaction is too big");
...
struct read_limiter {
    ...
    write(Sink& sink, const char* s, size_t count) {
        vros_ASSERT(_total + count <= Limit, tx_decompression_error, "Exceeded maximum decompressed transaction size");
        _total += count;
...
packed_transaction::get_raw_transaction() const {
    try {
        ...
        default:
            vros_THROW(unknown_transaction_compression, "Unknown transaction compression algorithm");
        }
    }
