...
transaction_context::init() {
    vros_ASSERT(!is_initialized, transaction_exception, "cannot initialize twice");
    vros_ASSERT(!trx.trx.actions.empty(), tx_no_action, "There's any actions in this transaction");
    ...
    if(!control.charge_free_mode()) {
        ...
        check_paid();    // Fail early if theren't no remainning avaiable vros & Pinned vros tokens
    }
    is_initialized = true;
}
...
transaction_context::exec() {
    vros_ASSERT(is_initialized, transaction_exception, "must first initialize");
...
transaction_context::check_time() const {
    ...
    if(BOOST_UNLIKELY(now > deadline)) {
        vros_THROW(deadline_exception, "deadline exceeded", ("now", now)("deadline", deadline)("start", start));
    }
}
...
transaction_context::check_charge() {
    ...
    if(charge > trx.trx.max_charge) {
        vros_THROW(max_charge_exceeded_exception, "max charge exceeded, expected: ${ex}, max provided: ${mp}",
...
transaction_context::check_paid() const {
    ...
    case address::reserved_t: {
        vros_THROW(payer_exception, "Reserved address cannot be used to be payer");
    }
    case address::public_key_t: {
        ...
        if(keys.find(payer.get_public_key()) == keys.end()) {
            vros_THROW(payer_exception, "Payer: ${p} needs to sign this transaction.", ("p",payer));
        }
        break;
    }
    case address::generated_t: {
        vros_ASSERT(payer.get_prefix() == N(domain), payer_exception, "Only domain generated address can be used to be payer");
        auto& key = payer.get_key();
        for(auto& act : trx.trx.actions) {
            vros_ASSERT(act.domain == key, payer_exception, "Only actions in '${d}' domain can be paid by it", ("d",act.domain));
        }
        break;
    }
    }  // switch
    ...
    asset vros, pvros;
    tokendb.read_asset_no_throw(payer, pvros_sym(), pvros);
    if(pvros.amount() > charge) {
        return;
    }
    tokendb.read_asset_no_throw(payer, vros_sym(), vros);
    if(pvros.amount() + vros.amount() >= charge) {
        return;
    }
    vros_THROW(charge_exceeded_exception, "There are only ${e} and ${p} left, but charge is ${c}", ("e",vros)("p",pvros)("c",charge));
}
...
transaction_context::record_transaction(const transaction_id_type& id, fc::time_point_sec expire) {
    try {
        ...
        vros_ASSERT(false, tx_duplicate,
