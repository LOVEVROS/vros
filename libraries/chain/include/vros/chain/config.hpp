...
namespace vros { namespace chain { namespace config {
...
const static uint128_t system_account_name = N128(vros);
...
const static int default_vros_link_expired_secs = 20;  // 20s -> total: 40s

}}}  // namespace vros::chain::config

template <typename Number>
Number
vros_PERCENT(Number value, int percentage) {
    return value * percentage / vros::chain::config::percent_100;
}

template <typename Number>
Number
vros_PERCENT_CEIL(Number value, uint32_t percentage) {
    return ((value * percentage) + vros::chain::config::percent_100 - vros::chain::config::percent_1)
           / vros::chain::config::percent_100;
}
